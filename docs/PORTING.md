...
|---|---------------------------------------------------------------|--------------------|---------------------------------------------|--------|-----------|
| 1 | `firmware/c/main.c`                                           | Rewrite            | `apps/faultycat_fw/main.c`                  | [~] provisional diag (F2b); full rewrite lands in F3 when the composite CDC replaces stdio_usb | Legacy `main` is a monolithic switch on a multicore FIFO — replaced by a single-core dispatcher + USB services. |
...
| 6 | `firmware/c/trigger.pio`, `trigger_basic.pio`                 | Port, tidy         | `drivers/emfi_pulse/trigger.pio`            | [ ] deferred to F4 | F2b uses CPU-timed fire (no PIO). PIO fire lands when the glitch engine's trigger compiler arrives. |
...
| 9 | `firmware/c/faultier/glitcher/ft_pio.{c,h}`                   | **Do not port**    | —                                           | [x] not ported (policy) | `hextreeio/faultier` has no LICENSE. Pattern reimplemented from scratch in F4/F5. |
| 10 | `firmware/c/faultier/glitcher/{trigger,delay,glitch}_compiler.c` | **Do not port** | —                                         | [x] not ported (policy) | Same. |

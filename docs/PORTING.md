...
| 14 | `firmware/c/serial/serial.c` (text console, command registry) | Partial rewrite    | `apps/faultycat_fw/main.c::process_shell_line` (per-service subshells: swd / jtag / scan / buspirate / serprog) | [x] F8-3 → F8-6 | Shell ended up in `apps/` rather than `services/pinout_scanner/shell.c` (the original plan filename was a shorthand). Unified parser dispatches by prefix into per-service handlers; output prefixes (`SWD:` / `JTAG:` / `SCAN:` / `SHELL:` / `BPIRATE:` / `SERPROG:`) keep `tools/{swd,jtag,scanner}_diag.py` decoupled. F8-4 / F8-5 plugged real binary-mode pumps into the placeholders; F8-6 added a mode-switch trailing-byte fix in `pump_shell_cdc` so `\r\n` doesn't bleed into the new parser. Per-command binary protocols still move to `host_proto/*` per the original plan. |
| 15 | `firmware/c/blueTag/` (submodule, v1.0.2)                     | Upgrade + port     | `third_party/blueTag/` (v2.1.2) + `services/{jtag_core, pinout_scanner, buspirate_compat, flashrom_serprog}` | [x] F8 closed (`v3.0-f8`) | MIT. Submodule upgraded at F0; jtag_core landed F8-1 (TAP), pinout_scanner F8-2 (P(8,4)/P(8,2) brute force + F8-6 stability check), buspirate_compat F8-4 (streaming BPv1 BBIO + OOCD), flashrom_serprog F8-5 (streaming serprog v1 + 4-pin SPI bit-bang). All four services attribute upstream MIT at file head; algorithmic shape preserved, blocking I/O reworked into streaming feed_byte / cooperative yield to fit the v3 main loop. |
...
| 23 | `third_party/blueTag/src/modules/flashProgrammer/serProg.{c,h}::{processCommands, get24bits, get32bits, S_CMD_*}` | Port (protocol + bus) | `services/flashrom_serprog/flashrom_serprog.{c,h}` | [x] F8-5 | blueTag MIT (attribution at flashrom_serprog.c head). Two structural changes vs upstream: (1) streaming `feed_byte` instead of blocking `getc(stdin)` + `fread(buffer, wlen)` — no 4 KB stage buffer, write bytes go to the wire as they arrive; (2) CPU bit-bang via callbacks instead of pico-sdk `spi_write_blocking` on fixed GP0..GP3 — the operator picks any 4 scanner channels at session entry. Implements: NOP, Q_IFACE, Q_CMDMAP, Q_PGMNAME ("FaultyCat"), Q_SERBUF (0xFFFF), Q_BUSTYPE (SPI-only), SYNCNOP, S_BUSTYPE, O_SPIOP (the workhorse), S_SPI_FREQ (we ack any non-zero and report 1 MHz back since bit-bang has no programmable clock), S_PIN_STATE (no-op + ack). Yield hook every 128 bytes of SPIOP traffic so multi-MB chip reads don't starve TinyUSB or the glitch campaigns. Programmer-name string changed from "blueTag" to "FaultyCat". |

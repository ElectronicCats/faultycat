...
bool crowbar_proto_feed(uint8_t byte, uint32_t now_ms) {
    if (s_state != S_SOF && (now_ms - s_last_byte_ms) > CROWBAR_PROTO_INTERBYTE_MS) {
        reset_parser();
    }
    ...
    switch (s_state) {
        ...
        case S_LEN_HI:
            ...
            if (s_len > CROWBAR_PROTO_MAX_PAYLOAD) {
                reset_parser();
                return false;
            }
            s_payload_pos = 0;
        ...
        case S_CRC_HI: {
            ...
            uint8_t hdr[3] = {s_cmd, (uint8_t)(s_len & 0xFFu), (uint8_t)((s_len >> 8) & 0xFFu)};
...
static size_t write_frame(uint8_t* out, size_t cap, uint8_t cmd_reply, const uint8_t* payload,
                          uint16_t len) {
    if (len > CROWBAR_PROTO_MAX_PAYLOAD)
        return 0;

...
> **JTAG / direct-SWD WIP cut 2026-05-19.** Por decisión del maintainer, todos los verbos JTAG (`jtag init/deinit/reset/trst/chain/idcode`) y todos los verbos SWD directos (`swd init/deinit/freq/connect/idcode/read32/write32/reset`) más `scan jtag` se consideran **WIP** y se ocultan de la superficie pública para `v3.0.0`. Único verbo público del scanner: `scan swd`. El cut es puramente de **dispatcher/CLI/TUI**: el firmware shell responde `<PREFIX>: ERR wip` para los verbos gated; las funciones `cmd_*` en `apps/faultycat_fw/main.c` quedan marcadas con macro `FW_WIP_UNUSED`; el host (`faultycmd.protocols.scanner`) retiene los wrappers como métodos privados `_swd_*` / `_jtag_*` / `_scan_jtag`. **No se tocan** `services/{swd_core, jtag_core, pinout_scanner, swd_bus_lock}`. Re-expose en v3.1 = revertir el patch del dispatcher + restaurar comandos CLI + páginas del modal TUI. Sub-fase F11-0d (Scanner control modal) **se reduce a un solo botón `Scan SWD`** en este release; los detalles del modal full-feature documentados abajo describen el estado final que volverá en v3.1.
